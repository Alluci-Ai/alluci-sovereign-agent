    ...
    def build_directive(
        ...
        if document_genre == DocumentGenre.BUSINESS_FINANCIAL:
            ...
            return (
                ...
                "DO NOT write a superficial summary. Enforce the 10-Layer Publication Monograph Architecture across granular, logically progressive thematic chapters:\n"
                "Enforce the Universal Hybrid Standard: Pair every high-density Markdown Table with 2-3 in-depth academic paragraphs directly below.\n"
                ...
                "4. Formal Mathematical State Space Modeling & Exact Derivations: Quote exact mathematical operators, state spaces $\\mathcal{S}$, transition kernels $\\mathcal{L}$, convolution integrals, and optimization bounds in standard LaTeX display math ($...$ and $$...$$). Explicitly distinguish authentic source equations from analytical reconstructs.\n"
                "5. Supporting Technical Figure Walkthroughs & Visual Asset Embeds: If extracted technical diagrams or figures are provided in the reference context, seamlessly cite and embed their exact Markdown image tags (e.g. `![Caption](/api/v1/artifacts/extracted_figures/...)`) alongside comprehensive structural walkthroughs explaining nodes, arrows, interaction loops, and formal dynamics.\n"
                "6. Domain-to-Domain Isomorphism Alignment Tables: Construct explicit tables mapping parameters and variables across domains (e.g. Dynamical States $\\leftrightarrow$ Quantum Observables).\n"
                "7. Taxonomical Decompositions & Logical Non-Implications: Construct category definition tables with explicit non-implication relations ($A \\not\\Rightarrow B$, e.g. $\\text{consciousness} \\not\\Rightarrow \\text{self}$).\n"
                "8. Dialectical Paradigm Audits: Systematically evaluate allied and rejected theories (e.g. GWT, HOT, FEP, AST, IIT, Searle) with precise functional reasons for agreement or rejection.\n"
                "9. Concrete Experimental Platforms & Computational Architectures: Formulate computational mechanisms for experimental platforms (e.g. NCAs, RCNs, attractor manifolds in artificial psychology).\n"
                "10. Empirical Falsification Criteria, Catalog of Open Limits & Condensed Mathematical Spine: Formulate numbered falsification criteria, the catalog of core theoretical bottlenecks/open problems, institutional governance safeguards (\\boxed{\\text{architect} \\neq \\text{sovereign}}), and compile the complete consolidated LaTeX equation block at the conclusion.\n\n"
                "STRICT FACTUAL GROUNDING LAWS:\n"
                ...
                "3. Quote exact mathematical definitions and equations when present; explicitly label formal models as reconstructions when conceptual.\n"
                "4. When technical figures are attached, you MUST embed their exact Markdown image tags directly into the relevant chapters.\n"
                "5. Maintain exhaustive academic depth, rigorous information density, and publication-grade LaTeX formatting throughout."
            )
