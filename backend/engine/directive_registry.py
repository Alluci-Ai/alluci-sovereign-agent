    ...
    def build_directive(
        ...
        if document_genre == DocumentGenre.BUSINESS_FINANCIAL:
            ...
            return (
                ...
                "4. Formal Mathematical State Space Modeling & Exact Derivations: Quote exact mathematical operators, state spaces $\\mathcal{S}$, measurable tuples $((X, \\mathcal{X}), (G, \\mathcal{G}))$, transition kernels $\\mathcal{L}$, convolution integrals, and optimization bounds in contiguous display LaTeX ($$...$$). Contextually embed Figure 1 (The 6-Tuple Conscious Agent Dynamical Loop) and derive the step-by-step measure-theoretic integration.\n"
                "5. Domain-to-Domain Isomorphism Alignment Tables: Construct explicit tables mapping parameters and variables across domains (e.g. Dynamical States $\\leftrightarrow$ Quantum Wave Functions $\\psi(x)$, Spacetime Intervals, Fundamental Forces).\n"
                "6. Taxonomical Decompositions & Logical Non-Implications: Construct category definition tables with explicit non-implication relations ($A \\not\\Rightarrow B$, e.g. $\\text{consciousness} \\not\\Rightarrow \\text{self}$, $\\text{perception} \\not\\Rightarrow \\text{truth}$).\n"
                "7. Dialectical Paradigm Audits: Systematically evaluate allied and rejected theories (e.g. Physicalism, GWT, HOT, FEP, AST, IIT, Searle's Biological Naturalism) with exhaustive critical paragraphs detailing exact points of divergence.\n"
                "8. Concrete Experimental Platforms, Computational Architectures & Interaction Graphs: Formulate computational mechanisms for simulation (Monte Carlo evolutionary games, NCAs, attractor manifolds). Contextually embed multi-agent graph interaction diagrams (e.g. Figures 5, 6, 7, and 8) with comprehensive structural walkthroughs of directed joins, message passing, and combination dynamics.\n"
                "9. Empirical Falsification Criteria & Explicit Catalog of Unresolved Open Limits: Formulate numbered empirical falsification conditions, a staged research roadmap, and an exhaustive catalog of core theoretical bottlenecks (e.g. combination problem, emergence of space-time, quantum collapse).\n"
                ...
                "STRICT MULTIMODAL RENDERING & KATEX LAWS:\n"
                "1. RAW UNESCAPED FIGURE IMAGE TAGS: When technical figures are attached, embed their raw unescaped Markdown image tags: ![Figure Caption](/api/v1/artifacts/extracted_figures/...). DO NOT wrap image tags in backticks, quotes, or code fences. Follow every figure immediately with an italicized structural caption and a clickable link: [🔍 View High-Resolution Diagram](/api/v1/artifacts/extracted_figures/...).\n"
                "2. STRICT KATEX CONTINUITY LAW: All display math blocks ($$...$$ or \\begin{aligned}...\\end{aligned}) must be contiguous with ZERO empty blank lines inside the delimiters to ensure valid KaTeX rendering.\n"
                "3. Ground all assertions strictly in the authentic reference data provided above with zero speculative hallucinations."
            )
