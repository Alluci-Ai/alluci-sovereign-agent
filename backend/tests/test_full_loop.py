...
async def test_objective_execute_success(mock_router, temp_db, mock_adapter_registry, mock_settings):
    ...
    with tempfile.TemporaryDirectory() as vdir, \
         ...
         patch.object(ExecutiveOrchestrator, '_perform_ppn_check', return_value=True):
        ...
        result = await orchestrator.execute_objective("Explain the Polytope protocol", AutonomyLevel.SEMI_AUTONOMOUS)
        ...
        assert summary["step1"] == "Adapter result"
