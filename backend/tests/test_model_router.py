...
# The sys.modules hacks are problematic for some libraries that inspect modules.
# We will use patch in the tests instead.
...
class TestModelRouterOllamaRoute:
    ...
    async def test_routes_to_ollama_when_ready(self):
        ...
        with patch.object(router, "_ollama_request", new_callable=AsyncMock) as mock_call:
