@pytest.mark.asyncio
async def test_channels_error_paths(app_client, mock_adapters):
    # Empty registry
    services.channel_registry.clear()
    assert app_client.get("/api/v1/channels/availability").json() == []

    # Missing adapter 404s
    assert app_client.get("/api/v1/channels/missing/config").status_code == 404
    assert app_client.put("/api/v1/channels/missing/config", json={}).status_code == 404
    assert app_client.post("/api/v1/channels/missing/connect").status_code == 404
    assert app_client.put("/api/v1/channels/missing/toggle").status_code == 404
    assert app_client.post("/api/v1/channels/missing/send", json={"recipient": "a", "content": "b"}).status_code == 404
    assert app_client.post("/api/v1/channels/missing/upload", json={"file_data": "a", "file_name": "b"}).status_code == 404
    assert app_client.get("/api/v1/channels/missing/health").status_code == 404
    assert app_client.get("/api/v1/channels/missing/unread").status_code == 404
    assert app_client.post("/api/v1/channels/missing/social", json={"task":"a", "params":{}}).status_code == 404
    assert app_client.post("/api/v1/channels/missing/enterprise", json={"task":"a", "params":{}}).status_code == 404
    ...
    assert app_client.put("/api/v1/channels/empty/config", json={}).status_code == 501
    assert app_client.post("/api/v1/channels/empty/connect").status_code == 501

    # Toggle connect unsupported
    EmptyAdapter.is_connected = False
    assert app_client.put("/api/v1/channels/empty/toggle").status_code == 501

    # Toggle disconnect unsupported
    EmptyAdapter.is_connected = True
    assert app_client.put("/api/v1/channels/empty/toggle").status_code == 501

    assert app_client.post("/api/v1/channels/empty/upload", json={"file_data": "a", "file_name": "b"}).status_code == 501
    assert app_client.post("/api/v1/channels/empty/social", json={"task":"a", "params":{}}).status_code == 501

    assert app_client.get("/api/v1/channels/empty/unread").json() == []

    # Send edge cases
    services.channel_registry["discord"] = mock_adapters["discord"]
    assert app_client.post("/api/v1/channels/empty/send", json={"recipient": "a", "content": "b"}).status_code == 503

    mock_adapters["discord"].is_connected = True
    assert app_client.post("/api/v1/channels/discord/send", json={"recipient": "a", "content": ""}).status_code == 400

    mock_adapters["discord"].send.side_effect = Exception("Send failed")
    assert app_client.post("/api/v1/channels/discord/send", json={"recipient": "a", "content": "b"}).status_code == 500

    mock_adapters["discord"].upload.side_effect = Exception("Upload failed")
    assert app_client.post("/api/v1/channels/discord/upload", json={"file_data": "a", "file_name": "b"}).status_code == 500

    mock_adapters["discord"].fetch_unread.side_effect = Exception("Unread failed")
    assert app_client.get("/api/v1/channels/discord/unread").status_code == 500

    mock_adapters["discord"].execute_social_task.side_effect = Exception("Task failed")
    assert app_client.post("/api/v1/channels/discord/social", json={"task": "a", "params": {}}).status_code == 500

    mock_adapters["discord"].execute_enterprise_task.side_effect = Exception("Task failed")
    assert app_client.post("/api/v1/channels/discord/enterprise", json={"task": "a", "params": {}}).status_code == 500
    ...
    health = app_client.get("/api/v1/channels/discord/health").json()
    ...
    assert app_client.post("/api/v1/channels/discord/social", json={"task": "send", "params": {"recipient": "a", "content": "b"}}).status_code == 200
    ...
    assert app_client.post("/api/v1/channels/discord/enterprise", json={"task": "other", "params": {}}).status_code == 200

@pytest.mark.asyncio
async def test_webhook_endpoints_error_paths(app_client, mock_adapters):
    services.channel_registry.clear()
    ...
    assert app_client.get("/api/v1/oauth/slack/start").status_code == 503
    ...
    assert app_client.get("/api/v1/oauth/instagram/start").status_code == 503
    ...
    assert app_client.get("/api/v1/oauth/facebook/start").status_code == 503
    ...
    assert app_client.get("/api/v1/oauth/x/start").status_code == 503
    assert app_client.get("/api/v1/oauth/msteams/start").status_code == 503
...
@pytest.mark.asyncio
async def test_specialized_channel_routes(app_client, mock_adapters):
    services.channel_registry.clear()
    # Missing adapter
    assert app_client.post("/api/v1/channels/iphone/pair", json={"cert": "123"}).status_code == 503
    assert app_client.post("/api/v1/channels/webchat/launch", json={"url": "abc"}).status_code == 501
    assert app_client.post("/api/v1/channels/icloud/2fa", json={"code": "123"}).status_code == 501
    assert app_client.post("/api/v1/channels/webchat/session/abc/capture", json={}).status_code == 501

    # iWatch
    assert app_client.get("/api/v1/channels/iwatch/status").json() == {"status": "unloaded"}
    assert app_client.get("/api/v1/channels/iwatch/pairing-qr").status_code == 503
    ...
    assert app_client.get("/api/v1/channels/iwatch/telemetry").status_code == 503

    # iphone pair failure
    mock_iphone = MockAdapter(store_pinned_ca=AsyncMock(return_value=False))
    services.channel_registry["iphone"] = mock_iphone
    assert app_client.post("/api/v1/channels/iphone/pair", json={}).status_code == 400
    assert app_client.post("/api/v1/channels/iphone/pair", json={"cert": "123"}).status_code == 500
    ...
    assert app_client.post("/api/v1/channels/webchat/launch", json={}).status_code == 400
    assert app_client.post("/api/v1/channels/webchat/launch", json={"url": "abc"}).status_code == 200
    assert app_client.post("/api/v1/channels/webchat/session/abc/capture", json={}).status_code == 200

    mock_icloud = MockAdapter(submit_2fa=AsyncMock(return_value={}))
    services.channel_registry["icloud"] = mock_icloud
    assert app_client.post("/api/v1/channels/icloud/2fa", json={"code": "123"}).status_code == 200

    # Delete agent subscription missing
    res = app_client.delete("/api/v1/agents/a1/subscriptions/c1")
