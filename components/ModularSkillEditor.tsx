...
// Inline StringArrayEditor for the Skill Editor
...
interface ModularSkillEditorProps {
    skill: any;
    onClose: () => void;
    onSaveGlobal: (updatedSkill: any) => Promise<void>;
    onForkAndAssign: (forkedSkill: any) => Promise<void>;
}

export const ModularSkillEditor: React.FC<ModularSkillEditorProps> = ({ skill, onClose, onSaveGlobal, onForkAndAssign }) => {
    ...
    const handleSaveGlobal = async () => {
        ...
        try {
            ...
            const res = await fetch(`${DAEMON_URL}/api/v1/skills/${localSkill.id}`, {
                method: 'PUT',
                headers: { 
                ...
                body: JSON.stringify(localSkill)
            });
            if (!res.ok) throw new Error("Failed to save global skill");
            await onSaveGlobal(localSkill);
        } catch (e) {
            console.error("Save global failed", e);
            alert("Failed to save global skill.");
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleFork = async () => {
        setForking(true);
        try {
            const newId = `${localSkill.id}_fork_${Date.now()}`;
            const forked = {
                ...localSkill,
                id: newId,
                name: `${localSkill.name} (Fork)`,
            };
            
            const token = localStorage.getItem('alluci_daemon_token');
            const csrfToken = await getCsrfToken(DAEMON_URL, token);
            const res = await fetch(`${DAEMON_URL}/api/v1/skills/${newId}`, {
                method: 'PUT',
                headers: { 
                    'Content-Type': 'application/json',
                    ...(token ? { 'Authorization': `Bearer ${token}` } : {}),
            ...
            if (!res.ok) throw new Error("Failed to fork skill");
    ...
    const updateArray = (key: string, arr: string[]) => {
        setLocalSkill({ ...localSkill, [key]: arr });
    };
    ...
    return (
        <div style={{
            ...
            <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--separator)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                    <div style={{ fontSize: 10, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>Skill Inspector</div>
                    <h2 style={{ margin: 0, fontSize: 18, fontWeight: 600, color: 'var(--text-primary)' }}>{localSkill.name}</h2>
                </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
                ...
                {Object.keys(localSkill)
                    .filter(key => Array.isArray(localSkill[key]) && key !== 'dependencies' && key !== 'capabilities')
                    .map(key => (
                    <EditorList 
                        key={key}
                        label={key.replace(/([A-Z])/g, ' $1').trim()} 
                        items={localSkill[key] || []} 
            ...
            <div style={{ padding: 20, borderTop: '1px solid var(--separator)', background: 'var(--fill-quaternary)', display: 'flex', gap: 12, flexDirection: 'column' }}>
                ...
                <button 
                    ...
                    className="glass-btn" 
                    style={{ padding: '10px 16px', fontSize: 14, width: '100%', display: 'flex', justifyContent: 'center' }}
                    title="Duplicate this skill to edit it safely just for this agent."
                >
